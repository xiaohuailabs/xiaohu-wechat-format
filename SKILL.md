...
- 排版完成后，要优先给用户一个已经主动测试过的本地 `127.0.0.1` 预览地址。
- 同时也要明确告诉用户，本地 `wechat output/` 里还生成了一份可直接打开的文件 URI，例如 `file:///D:/IP_in_IDE/AI/vibe%20coding/wechat%20output/gallery.html`。
- 给结果时不要只报一个地址。要把本地服务地址和本地文件地址都说清楚，提升可用性和稳定性。
...
- 如果是 Gallery 模式，优先反馈一个已经测试通过的 `127.0.0.1` 地址，同时补充 `wechat output/` 下的本地文件 URI，明确这两个地址都能打开
